...
app.use(cookieSession({
    name: 'session',
    keys: ['key1', 'key2'],
    maxAge:  3600 * 15 // set life cycle session
}));
...
// REGISTER PAGE
app.post('/register_user', ifLoggedin, 
// post data validation(using express-validator)
[   body('user_name','Please Enter Your Name!').trim().not().isEmpty(),
    body('user_lastname','Please Enter Your Last Name!').trim().not().isEmpty(),
    body('user_username','Invalid Username!').trim().not().isEmpty().custom((value)=>{
    ...
    //body('user_username','The Username must be maximun length 12 Characters').trim().not().isLength({ max:12 }),
    body('user_pass','The password must be of minimum length 6 Characters').trim().isLength({ min: 6 }),
],// end of post data validation
(req,res,next) => {

    const validation_result = validationResult(req);
    const {user_name, user_lastname, user_pass, user_username} = req.body;
